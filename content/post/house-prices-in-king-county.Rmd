...
packages: ['mlr3', 'mlr3learners', 'mlr3viz', 'mlr3filters', 'mlr3tuning', 'mlr3data', 'mlr3book', 'paradox', 'ggmap', 'GGally', 'mlr3db', 'mlr3proba', 'ranger']
---
...
We simply replace it to days since the first day recorded in the data set:

```{r, message = FALSE}
dates = kc_housing$date
kc_housing$date = as.numeric(difftime(dates, min(dates), units = "days"))
```

Next, we convert the zip code to a factor:

```{r}
kc_housing$zipcode = as.factor(kc_housing$zipcode)
```

We replace the missing values in column **renovated** (indicating that a house has not been renovated yet) with the number 0, and add the logical column **renovated**:

```{r}
kc_housing$renovated = !is.na(kc_housing$yr_renovated)
kc_housing$yr_renovated[!kc_housing$renovated] = 0
```

We do the same for missing values in variable **sqft_basement** for houses without a basement:
```{r}
kc_housing$basement = !is.na(kc_housing$sqft_basement)
kc_housing$sqft_basement[!kc_housing$basement] = 0
```
